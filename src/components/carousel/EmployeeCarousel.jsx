  ...
  return (
    <div className="mr-[23px]">
      <div className="items-center w-full h-[72px]  border border-[#E4E7EC] rounded-[12px] p-[16px] flex gap-[12px] bg-white pr-8">
        <img
          src={data?.profileUrl ?? "https://picsum.photos/200"}
          alt=""
...
const EmployeeCarousel = ({ data }) => {
  const slides = data?.length > 3 ? 4 : data?.length;
  const settings = {
    slidesToShow: slides,
    slidesToScroll: 1,
    isFinite: data?.length > 3 ? true : false,
    className: data?.length > 3 ? "" : "max-w-[231px]",
  };
